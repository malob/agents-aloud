...
final class ClaudeCLISpeechProcessor: SpeechTextProcessor {
    ...
    // System prompt is shared with CodexCLISpeechProcessor and lives
    // in `SpeechRewriterInstructions` (Sources/Models/) — see that
    // file for the load-bearing rules and their rationale. We
    // deliver it to the Claude CLI via `--system-prompt`.
